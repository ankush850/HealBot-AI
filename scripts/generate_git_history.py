...
# 1. Ensure git is clean or re-initialized
print("Configuring git repo...")
run_cmd("git config user.name")
# Check if user.name is set, otherwise set standard defaults if missing
res = run_cmd("git config user.name")
...
# Generate 254 timestamps evenly distributed with slight random jitter
...
for i in range(TOTAL_COMMITS):
    ...
    # Add random jitter of +/- a few hours/minutes to feel realistic
...
# Pre-defined realistic commit categories and messages
commit_messages = [
    # Phase 1: Setup & Initial Architecture (Commits 1-25)
    ...
    # Phase 2: Core Enhancements & Optimization (Commits 26-100)
    ...
    # Phase 3: Advanced Features & Refinements (Commits 101-180)
    ...
    # Phase 4: Polish, Branding & Production Readiness (Commits 181-254)
    ...
    "docs: add comprehensive CHANGELOG.md for release v2.0.0",
    ...
    "feat(release): HealBot AI v2.0.0 ready for production deployment"
]

# Ensure we have exactly 254 messages
...
# Phase 1 files to add gradually in first 12 commits
initial_stages = [
    [".gitignore"],
    ["requirements.txt"],
...
print(f"Creating {TOTAL_COMMITS} commits from {START_DATE.date()} to {END_DATE.date()}...")

# Stage 0: Add initial files progressively
...
# Remaining commits (from len(initial_stages) up to TOTAL_COMMITS)
# For each subsequent commit, we will add subtle updates (e.g. updating a tracking / changelog file, docs, comments)
changelog_path = os.path.join(REPO_DIR, "CHANGELOG.md")
with open(changelog_path, "w", encoding="utf-8") as f:
    f.write("# HealBot AI Changelog\n\nAll notable changes to HealBot AI are documented here.\n\n")
...
for i in range(len(initial_stages), TOTAL_COMMITS):
    ...
    # Append entry to CHANGELOG.md or update minor doc/comment
    with open(changelog_path, "a", encoding="utf-8") as f:
        f.write(f"- **{ts.strftime('%Y-%m-%d %H:%M')}**: {msg}\n")
    
    run_cmd(f"git add CHANGELOG.md")
    
    # Also stage everything if any new files were added
...
print(f"Total commits created: {res.stdout.strip()}")
